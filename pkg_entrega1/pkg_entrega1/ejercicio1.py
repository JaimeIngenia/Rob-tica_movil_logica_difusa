...
class SensorLidar(Node):
    ...
    def controlVel(self):
        ...
        if self.parar==0:   # Si el robot esta en stop 
            ...
            #print("RANGOS",self.rangos[0], self.dist_limit)

            if self.rangos[0] < self.dist_limit:  
                vel.linear.x = 0.0  
                vel.angular.z = 0.5
                if self.giro==0:
                    self.rotacion()
    ...
    def mostrar_datos(self):
        ...
        ["Velocidad", vel],
        ["Aceleración", acel],
        ["Distancia de objeto cercano", dis_obj]
        ]
        table = tabulate(table_data, headers=["Parámetro", "Valor"], tablefmt="fancy_grid")
        print(table)
