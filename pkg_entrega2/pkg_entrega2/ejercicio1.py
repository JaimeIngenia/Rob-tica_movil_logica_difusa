...
class Camara(Node):
    ...
    def pidController(self, error):
        Kp = 0.0008#0.0008  #ESta relacionado con la diferencia del valor deseado y el error
        Ki = 0.00000 #acumulacion de errores
        Kd = 0.0002 #0.00 #velocidad de respuesta #predice errores futuros
        ...
        #print("P:" + P + "\n" + "I:" + I + "\n" + "D:" + D + "\n" + "Control señal:" + control_signal )
        #mensaje = "P:" + str(P) + "\n" + "I:" + str(I) + "\n" + "D:" + str(D) + "\n" + "Control señal:" + str(control_signal)
        #print(mensaje)
        print("--------------")
    ...
    def updateCallback(self):
        ...
        if self.EstadoInicial is True:
            self.detectLane()
            self.get_logger().info(f"centro Error: {self.error}")
            c = self.pidController(self.error)
            vel.angular.z = c
            if (vel.angular.z > 0.5 or vel.angular.z < -0.3  ):
                #print("Organizando vel angular")
                c=0.18
                
            x = abs(self.center_error - self.error)
            
            self.variableString=str(self.error)
            self.variableString_vel=str(vel.linear.x )
            self.variableString_c=str(c )

            #print("Velocidad Angular" + self.variableString_c)
            #print("error" + self.variableString)
            #print("velocidad lineal en x:::***:::" + self.variableString_vel )

            if self.error > 100 or self.error < -100:
                #print("velocidad lineal entrando al if:" + self.variableString_vel)
                y = 0.015
            # elif self.error > 400 or self.error < -400:
            #     print("velocidad Fernand_ENtrò al if:" + self.variableString_vel)
            #     y = 0.015
            else:
                y = 0.2
            vel.linear.x = y
...
## shape[1] ancho
        ...
        #print("VAriable julian: error*******************" )
        #print(self.error) #Andrès
